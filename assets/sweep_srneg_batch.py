#!/usr/bin/env python3
"""Batch sweep stage-2 pull-down strength for SR- tuning."""
import os
import sys
import tempfile
...
def build_netlist(base, points):
    first = points[0]
    lines = []
    lines.append('* Two-Stage OTA - SR- stage2 pull-down sweep via .alter')
...
def _score(m):
    return (
        abs(m['sr_neg_vus'] - 5.0) * 1.2
        + abs(m['sr_pos_vus'] - 5.0) * 0.8
        + max(0.0, 80.0 - (m['hold_ns'] or 0.0)) / 20.0
        + abs(m['overshoot_v']) * 10.0
        + max(0.0, 0.15 - m['n2_min_v']) * 10.0
    )


def run_sweep(base, points):
    netlist = build_netlist(base, points)
    with tempfile.NamedTemporaryFile(mode='w', suffix='.sp', delete=False, encoding='utf-8') as f:
        f.write(netlist)
        tmp = f.name
    try:
        lis = run_hspice(tmp, 'batch_srneg_sweep', timeout=600)
    finally:
        os.unlink(tmp)
    ...
    for p, block in zip(points, _blocks(arr)):
        ...
        m['score'] = _score(m)
        results.append(m)
    for f in Path(lis).parent.glob('batch_srneg_sweep*'):
        f.unlink(missing_ok=True)
    results.sort(key=lambda x: x['score'])
    print(f"{'m6':>3s} {'m7':>3s} {'SR+':>5s} {'SR-':>5s} {'hold':>7s} {'Ovsh':>6s} {'N2min':>6s} {'score':>6s}")
    print('-' * 52)
    for r in results:
        print(f"{r['m_stage2']:3d} {r['m_p2']:3d} {r['sr_pos_vus']:5.2f} {r['sr_neg_vus']:5.2f} {r['hold_ns']:7.1f} {r['overshoot_v']:6.3f} {r['n2_min_v']:6.3f} {r['score']:6.2f}")
    return results
