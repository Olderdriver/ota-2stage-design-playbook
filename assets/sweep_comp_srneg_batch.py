#!/usr/bin/env python3
"""Batch sweep Cc/Rz around current strong-drive point for SR- tuning."""
import os
import sys
import tempfile
...
def build_netlist(base, points):
    first = points[0]
    lines = []
    lines.append('* Two-Stage OTA - compensation retune for SR- via .alter')
...
def _score(m):
    return (
        abs(m['sr_neg_vus'] - 5.0) * 1.1
        + abs(m['sr_pos_vus'] - 5.0) * 0.7
        + max(0.0, 100.0 - (m['hold_ns'] or 0.0)) / 25.0
        + abs(m['overshoot_v']) * 10.0
    )


def run_sweep(base, cc_values, rz_values):
    points = [{'cc_pf': cc, 'rz_kohm': rz} for cc in cc_values for rz in rz_values]
    netlist = build_netlist(base, points)
    with tempfile.NamedTemporaryFile(mode='w', suffix='.sp', delete=False, encoding='utf-8') as f:
        f.write(netlist)
        tmp = f.name
    try:
        lis = run_hspice(tmp, 'batch_comp_srneg_sweep', timeout=600)
    finally:
        os.unlink(tmp)
    ...
    for p, block in zip(points, _blocks(arr)):
        ...
        m['score'] = _score(m)
        results.append(m)
    for f in Path(lis).parent.glob('batch_comp_srneg_sweep*'):
        f.unlink(missing_ok=True)
    results.sort(key=lambda x: x['score'])
    print(f"{'Cc':>4s} {'Rz':>5s} {'SR+':>5s} {'SR-':>5s} {'hold':>7s} {'Ovsh':>6s} {'N2min':>6s} {'score':>6s}")
    print('-' * 50)
    for r in results:
        print(f"{r['cc_pf']:4.1f} {r['rz_kohm']:5.1f} {r['sr_pos_vus']:5.2f} {r['sr_neg_vus']:5.2f} {r['hold_ns']:7.1f} {r['overshoot_v']:6.3f} {r['n2_min_v']:6.3f} {r['score']:6.2f}")
    return results
...
if __name__ == '__main__':
    ...
    run_sweep(base, [2.0, 2.5, 3.0, 3.5, 4.0], [6.8, 9.4, 12.0, 15.0, 18.0])
